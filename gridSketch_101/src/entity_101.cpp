...
Entity_101::Entity_101(){
}

void Entity_101::setup(int _cell_i, int _n_rows, int _n_cols){
    // GLOBALS
    ...
    // 1/4 chance to get darker color
    if (ofRandom(1.0) < 0.25) {
        color.set(ofRandom(100),ofRandom(100),ofRandom(100));
    } else {
...
void Entity_101::draw(){
    ...
    // Get the absolute position of the Entity_101 in the screen
    ...
    ofDrawRectangle(abs_pos_x, abs_pos_y,
                    width_ent * SIDE_CUBE_w,
                    height_ent * SIDE_CUBE_h);
    
}

int Entity_101::getRow(int i, int n_cols) {
    int row = (int)i / n_cols;
    return row;
}

int Entity_101::getCol(int i, int n_cols) {
    int col = (int)i % n_cols;
    return col;
}

int Entity_101::getCell(int row, int col, int n_cols) {
    int cell = (int)(row * n_cols) + col;
    return cell;
}

int Entity_101::giveRandomIndex(int n_options) {
  return (int)ofRandom(n_options);
}

vector<int> Entity_101::change(vector<int> input) {
